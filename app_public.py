...
def show_chat_interface():
    ...
    st.markdown("""
    <div class="main-title">🏛️ Turnos Cédulas</div>
    ...
    # Input del usuario - USANDO TEXT_INPUT EN LUGAR DE CHAT_INPUT
    ...
    if submit and prompt:
        ...
        st.rerun()  # Rerun para mostrar el mensaje del usuario
    
    # Procesar respuesta del bot si hay un mensaje pendiente
    if len(st.session_state.messages) > 0:
        last_message = st.session_state.messages[-1]
        
        # Si el último mensaje es del usuario y no tiene respuesta
