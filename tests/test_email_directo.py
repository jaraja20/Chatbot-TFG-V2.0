...
print("[SEARCH] DIAGNÓSTICO DE ENVÍO DE EMAIL")
print("=" * 60)

# 1. Verificar variables de entorno
print("\n1[*]⃣ Verificando configuración:")
...
if not smtp_email or not smtp_password:
    print("\n[FAIL] ERROR: Credenciales de email no configuradas en .env")
    sys.exit(1)

# 2. Verificar que el módulo email_sender existe
print("\n2[*]⃣ Verificando módulo email_sender:")
try:
    from email_sender import EmailNotificationSender
    print("   [OK] Módulo email_sender encontrado")
except ImportError as e:
    print(f"   [FAIL] ERROR: No se puede importar email_sender: {e}")
    print(f"   [*] Buscando en: {os.path.join(os.path.dirname(__file__), '..', 'notificaciones')}")
    sys.exit(1)

# 3. Crear instancia del sender
print("\n3[*]⃣ Creando EmailNotificationSender:")
try:
    ...
    print("   [OK] EmailNotificationSender creado correctamente")
except Exception as e:
    print(f"   [FAIL] ERROR al crear EmailNotificationSender: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)

# 4. Preparar datos de prueba
print("\n4[*]⃣ Preparando datos de prueba:")
turno_data = {
...
try:
    ...
    print(f"   [OK] QR generado correctamente")
except Exception as e:
    print(f"   [WARN] WARNING: No se pudo generar QR: {e}")
    qr_data = None

# 6. Enviar email de prueba
print("\n6[*]⃣ Enviando email de prueba:")
