...
def test_connection():
    ...
    logger.info("🔍 PRUEBA 1: Conexión a base de datos")
...
def test_tables():
    ...
    logger.info("🔍 PRUEBA 2: Estructura de tablas")
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            logger.info(f"✅ Tabla 'turnos' encontrada con {len(columnas)} columnas:")
            for col in columnas:
...
def count_existing_turnos():
    ...
    logger.info("🔍 PRUEBA 3: Conteo de turnos existentes")
    ...
    try:
        with get_db_session() as session:
            ...
            logger.info(f"✅ Total de turnos: {total}")
            logger.info(f"✅ Turnos activos: {activos}")
            logger.info(f"✅ Turnos futuros: {futuros}")
            
            # Mostrar algunos turnos recientes
            turnos_recientes = session.query(Turno).order_by(Turno.created_at.desc()).limit(5).all()
            logger.info(f"✅ Últimos 5 turnos:")
...
def create_test_data():
    ...
    logger.info("🔍 PRUEBA 4: Creando datos de prueba")
    ...
    try:
        with get_db_session() as session:
            ...
            logger.info(f"✅ Datos de prueba creados: {datos_creados} turnos")
...
def test_ocupacion_functions():
    ...
    logger.info("🔍 PRUEBA 5: Funciones de ocupación")
    ...
    try:
        with get_db_session() as session:
            ...
            logger.info(f"✅ Disponibilidad por franjas:")
        ...
        logger.error(f"❌ Error probando funciones de ocupación: {e}")
        import traceback
...
def test_fuzzy_detection():
    ...
    logger.info("🔍 PRUEBA 6: Detección de frases ambiguas")
    ...
    try:
        for frase in frases_test:
            ...
            logger.info(f"✅ '{frase}' -> {status}")
        
        return True
        
    except Exception as e:
        logger.error(f"❌ Error probando detección difusa: {e}")
        return False

def test_database_performance():
    """Prueba el rendimiento de consultas a la BD"""
    logger.info("🔍 PRUEBA 7: Rendimiento de BD")
    ...
    try:
        ...
        with get_db_session() as session:
            ...
            logger.info(f"✅ Consultas completadas en {elapsed:.2f} segundos")
            
            if elapsed > 5:
                logger.warning("⚠️ Las consultas son lentas, considera optimizar")
            
            return True
            
    except Exception as e:
        logger.error(f"❌ Error probando rendimiento: {e}")
        return False

def generate_availability_report():
    """Genera un reporte detallado de disponibilidad"""
    logger.info("🔍 PRUEBA 8: Reporte de disponibilidad")
    
    try:
        with get_db_session() as session:
            hoy = datetime.date.today()
            logger.info(f"\n📊 REPORTE DE DISPONIBILIDAD - {hoy}")
            logger.info("=" * 50)
            ...
            for i in range(1, 6):
                ...
                if horarios:
                    ...
                    logger.info(f"   ⚠️ Sin horarios disponibles")
...
def run_full_diagnostic():
    ...
    if passed == total:
        ...
        logger.info("💡 El motor difuso debería funcionar perfectamente")
    else:
        logger.warning("⚠️ SISTEMA CON PROBLEMAS DETECTADOS")
        logger.info("🔧 Revisa los errores arriba para solucionarlos")
...
if __name__ == "__main__":
    print("🔍 Script de Diagnóstico del Sistema de Turnos")
    print("=" * 50)
